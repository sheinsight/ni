...
if [ "$OS" = "Darwin" ]; then
  if [ "$ARCH" = "x86_64" ]; then
    BINARY_URL="https://github.com/sheinsight/ni/releases/download/$V/x86_64-apple-darwin.tar.gz"
    INSTALL_DIR="/usr/local/bin"
  elif [ "$ARCH" = "arm64" ]; then
    BINARY_URL="https://github.com/sheinsight/ni/releases/download/$V/aarch64-apple-darwin.tar.gz"
    INSTALL_DIR="$HOME/.local/bin"
  fi
elif [ "$OS" = "Linux" ]; then
  BINARY_URL="https://github.com/sheinsight/ni/releases/download/$V/x86_64-unknown-linux-musl.tar.gz"
  INSTALL_DIR="$HOME/.local/bin"
elif [ "$OS" = "MINGW*" ]; then
  BINARY_URL="https://github.com/sheinsight/ni/releases/download/$V/x86_64-pc-windows-gnu.tar.gz"
  INSTALL_DIR="$HOME/.local/bin"
else
